...
class MapChart extends PureComponent {
  ...
  render() {
    ...
    let mapOption = cityOption;
    if (this.props.isProvince) {
      mapOption = provinceOption;
      ...
      data = data.map((item, index) => {
        const newItem = {};
        newItem.name = item.key.replace('市', '').replace('省', '');
      ...
      mapOption.series[1].data = data.slice(0, 20).reverse();
      mapOption.yAxis.data = nameList.slice(0, 20).reverse();
